...
// 请求json数据
async function load() {
    musics = await Promise.resolve(request.get("data/task.json"));
}
function createList() {
    ...
    <div class="imglist-item">
      ...
      <span class="play-count">
          <span>播放量：</span>
          <span class="play-count-data">${play.listen_num >= 10000 ? (play.listen_num / 10000).toFixed(1) + '万' : play.listen_num}</span>
      </span>
    </div>`;
    }
    // 中部
    ...
    <div class="song-content">
        ...
        <div class="text-content">
            ...
            <span class="content-right">${('0' + Math.floor(song.interval / 60) + ':') + (song.interval % 60 >= 10 ? song.interval % 60 : '0' + song.interval % 60)}</span>
        </div>
    </div>
    `;
    }
    ...
    <div class="mv-item">
        ...
        <span class="watch-count">
            ...
            <span>${mv.listen_num >= 10000 ? (mv.listen_num / 10000).toFixed(1) + '万' : mv.listen_num}</span>
        </span>
    </div>
    `;
    }
